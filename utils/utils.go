...
//获取ip int64
func GetIntIP() (int64, error) {
	i, err := GetIP()
	if err != nil {
		return 0, err
	}
	ret := big.NewInt(0)
	ret.SetBytes(net.ParseIP(i.String()).To4())
...
//count 总数
//limit 每页个数
func GetPages(count, Limit int64) int64 {
	return int64(math.Ceil(float64(count) / float64(Limit)))
}

//获取每个月的第一天
func GetFirstDateOfMonth(timeData int64) time.Time {
	d := time.Unix(timeData, 0)
...
func SliceRemoveDuplicate(languages []string) []string {
...
func GetCname(src string) (dst []string) {
	c := dns.Client{
		Timeout: 5 * time.Second,
	}
	res := []string{}
	dnsServices := []string{"8.8.8.8", "208.67.222.222", "114.114.114.114", "223.5.5.5"}
	for _, dnsService := range dnsServices {
		m := dns.Msg{}
		m.SetQuestion(src+".", dns.TypeA)
		r, _, errExchange := c.Exchange(&m, dnsService+":53")
		if errExchange != nil {
			continue
		}
